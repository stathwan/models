...
class sampling(Layer):
    ...
    def call(self, theta):
        ...
        _z=_mu_hat +K.exp(-10. / 2)*_epsilon
        return  _z   
...
class VAELoss():
    ...
    def convloss(self,x, x_hat):
        ...
        KLDivergence = 0.5*K.sum(1.+ -10. - K.square(mu_hat) - K.exp(-10.), axis=-1)
...
convae.load_weights('c:/data/vae/vae_model_-10.hdf5')
#encoder
encoder = Model(x, z)
##visualization
...
# we will sample n points within [-15, 15] standard deviations
grid_x = np.linspace(-1, 1, n)
grid_y = grid_x*0.16-0.25
#grid_y = grid_x*0.3 +1.5
